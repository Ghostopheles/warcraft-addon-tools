...
class LuaCheckPattern:
    ...
    def __init__(self, pattern:str):
        ...
        patterns = self.pattern.split(self.SEPARATOR)
        self.regex_patterns = [re.compile(pattern) for pattern in patterns]

        self.max = len(self.regex_patterns)
...
class LuaCheckParser:
    def __init__(self, luacheck_file:str=None):
    ...
    def parse_luacheck_file(self):
        with open(self.luacheck_file, "r") as luacheck:
            ...
            # get globals table line numbers, this might be terrible but it works (sort of)
            for line in lines:
