...
class LibFetch:
    ...
    def parse_yaml(self):
        ...
        yaml_data = YamlParser(yaml_file)

        if not yaml_data: raise Exception(f"Error occurred when parsing {self.dotfile_name} file.")
    ...
    def fetch_git_repo(self, name:str, url:str):
        ...
        try:
            ...
            if new_url:
                ...
                command += url
            subprocess.run(command, capture_output=True, shell=True, cwd=os.path.dirname(repo_path))
    ...
    def move_lib_folders(self):
        ...
        for src, dest in self.yaml_move_folders.items():
            source, destination = os.path.join(self.checkout_path, src), os.path.join(self.checkout_path, dest)

            #copy the folders to their respective destinations from the yaml file - 'move-folders'
            if os.path.exists(source):
                shutil.copytree(source, destination, ignore=ignore_pattern, dirs_exist_ok=True)
        ...
        shutil.copytree(self.checkout_path, self.repo_path, ignore=ignore_pattern, dirs_exist_ok=True)
    ...
    def cleanup(self):
        if self.moved_files:
            ...
            raise Exception(f"Attempted to delete .checkout folder before files had been successfully moved.")
